...
interface GroupProductCardProps {
  title: string;
  api?: string; // optional since we’re not using it
}

const GroupProductCard: React.FC<GroupProductCardProps> = ({ title }) => {
  const navigate = useNavigate();
  const [products, setProducts] = useState<ProductItem[]>([]);

  useEffect(() => {
    const staticProducts: ProductItem[] = [
      { id: 1, title: "Slim Laptop Pro", image: product1, price: 45999 },
      { id: 2, title: "Gaming Beast 3000", image: product2, price: 78999 },
      { id: 3, title: "Everyday Notebook", image: product3, price: 34999 },
      { id: 4, title: "UltraThin Air", image: product4, price: 65999 },
    ];

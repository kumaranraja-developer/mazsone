  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then((res) => {
        ...
        const imageList = data.images?.length
          ? data.images.map((img: string) => `http://127.0.0.1:8000/${img}`)
          : [`http://127.0.0.1:8000/${data.image}`];

        setSelectedImage(imageList[0]);
        setProduct((prev) => prev && { ...prev, images: imageList });
      })
      .catch((err) => {
        console.error(err);
  ...
  <div className="flex flex-col md:flex-row gap-4 justify-center items-center">
    ...
    <div className="hidden md:block">
      <VerticalImageList
        ...
        selectedIndex={(product.images || []).findIndex((img) => img === selectedImage)}
